...
Max_Depth = 2          #Change this variable to corrispond with the epsilon value wanted
...
DecisionTree_Reg = DecisionTreeRegressor(max_depth = Max_Depth )
DecisionTree_Reg.fit(X_train,y_train)

#predicting values for test set
start=timeit.default_timer()  
DecisionTree_predicted = DecisionTree_Reg.predict(X_test)

#finding Mean squared error
DecisionTree_rmse = np.sqrt(mean_squared_error(y_test, DecisionTree_predicted))
stop = timeit.default_timer()

#reformating input data for graphing
x_test = X_test["x_1"]
Y_test = X_test["x_2"]
differance = DecisionTree_predicted - y_test
...
surf_prediction= ax_prediction.plot_trisurf(x_test,Y_test, DecisionTree_predicted, cmap=cm.coolwarm,linewidth=0, antialiased=False)
...
#cross validation 
scores = cross_val_score(DecisionTree_Reg,X,y,scoring="neg_mean_squared_error",cv=10)
DecisionTree_CV = np.sqrt(-scores) 


## EXECUTABLE ITEMS

#predicted values
print("prediction values",DecisionTree_predicted)
# RMSE of cross validation
print("RMSE for set",DecisionTree_rmse)
# time prediction/execution
print('Time for prediction: ', stop - start)
# Cross validation information
print("the value of cross validation value",DecisionTree_CV.mean())
#show graph
plt.show()